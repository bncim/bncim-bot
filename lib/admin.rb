...
class AdminPlugin
  ...
  def servers(m)
    if m.channel == "#bnc.im-admin"
      ...
      ips.each do |name, addrs|
        ipv4 = addrs["ipv4"]
        ipv6 = addrs["ipv6"]
        reply = "#{Format(:bold, "[#{name}:#{$userdb.servers[name].networks_count.to_s.rjust(2, '0')}]")} " + \
  ...
  def network_view(m, network)
    ...
    ips.each do |name, addrs|
      ...
      unless ipv6.nil?
        ipv6.each do |ip|
          reply = reply + "#{name}-6-#{ipv6.index(ip)} (#{servers[name].conns_on_iface(ip, network).to_s.rjust(2, '0')}), "
        end
      end
      m.reply reply[0..-3]
    end
